...
PER_PAGE = 200
REQUEST_DELAY = 1.0
MAX_RESULTS = 0  # 0 = tidak terbatas
ISSN_BATCH_SIZE = 50
...
def extract_article_data(work, issn_electronic_set):
    try:
        ...
        # Cari ISSN yang sesuai dengan daftar kita
        issn_electronic = ""
...
def build_search_query(keywords, mode="all"):
    """
    Membangun query boolean untuk parameter search OpenAlex.
    mode "all" -> keyword1 AND keyword2 AND ...
    mode "any" -> keyword1 OR keyword2 OR ...
    """
    if not keywords:
        return ""
    if mode == "all":
...
def fetch_group(group_name, group_config, issn_list, year_from, year_to, lang_list, per_page, delay, max_results):
    """
    Fetch artikel untuk SATU group pencarian menggunakan parameter search.
    Mengembalikan list artikel (dictionary) dan jumlah total yang berhasil diambil.
    """
    keywords = group_config["keywords"]
    mode = group_config.get("mode", "all")
    search_query = build_search_query(keywords, mode)
    if not search_query:
        print(f"  Kelompok '{group_name}' tidak memiliki keyword, dilewati.")
        return [], 0
    ...
    # Siapkan batch ISSN (karena URL terbatas)
    ...
    for batch_idx, batch in enumerate(issn_batches):
        ...
        while cursor:
            ...
            try:
                ...
                for work in results:
                    # Batasi jika MAX_RESULTS terpenuhi (per group)
                    ...
                    # Ekstrak data, lewati jika gagal
...
def main():
    print("=" * 60)
    print("  OpenAlex Article Fetcher — Optimized dengan Search Parameter")
    print("=" * 60)
    print()
    ...
    for idx, (name, config) in enumerate(SEARCH_GROUPS.items(), 1):
        print(f"  {idx}. {name} → keywords: {config['keywords']} (mode: {config.get('mode','all')})")
    print()

    print("=" * 60)
    print("  Mulai fetch data per kelompok (dengan filter search dari API)...")
    print("=" * 60)
    print()
    ...
    for group_name, group_config in SEARCH_GROUPS.items():
        ...
        time.sleep(REQUEST_DELAY)  # jeda antar group
